...
const CURRENT_USER_ID = "usr-99";
const CURRENT_USER_NAME = "Elman Pathan";
...
const MOCK_IDEA = {
  ...
  problem:
    "Indie hackers spend months building solutions without verifying actual buyer intent signals first, causing severe resource drain.",
  solution:
    "A low-latency matrix analysis node that maps keyword trend variations against open payment gateway APIs.",
};
...
const MOCK_COMMENTS = [
  {
    ...
    text: "This architecture scales beautifully. Definitely needed.",
    timestamp: "2026-05-18 14:22",
  },
  {
    id: "c2",
    userId: "usr-99",
    userName: "Elman Pathan",
...
export default function IdeaDetailsView() {
  ...
  const params = useParams(); // Dyn-parameter hook instance

  // Unwrap dynamic routing param structure
  const ideaId = params?.id || "1";

  const [comments, setComments] = React.useState(MOCK_COMMENTS);
  const [newCommentText, setNewCommentText] = React.useState("");

  // States to monitor active inline editing
  ...
  return (
    <div className="flex-1 w-full max-w-4xl mx-auto px-4 py-8 flex flex-col gap-6">
      ...
      {/* DETAILED CONTENT DISPLAY COMPONENT */}
      <div className="p-6 rounded-xl border border-zinc-200 bg-card dark:border-zinc-800 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3 pt-2 text-xs leading-relaxed">
          <div>
            <h3 className="font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider text-[10px] text-zinc-400 mb-1">
              Problem Statement
            </h3>
      ...
      {/* COMMENT LOG WORKFLOW CONTAINER */}
          ...
          {comments.map((comment) => (
            ...
            >
              <div className="flex items-center justify-between w-full text-[11px]">
                ...
                {/* OWNER INTERACTION ACCESS CONTROL RIGHTS */}
              ...
              {/* RENDER BODY LOOP OR TEXTAREA SWITCH CONTEXT */}
